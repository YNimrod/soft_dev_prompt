...
| 2026-06-20 | 00_grill_me.md（Phase 0）に「誰のために」先鋭化の観点を追加。既存の「曖昧点の洗い出し→質問」フロー内に統合し、対象者が広すぎる場合はそれを最優先の質問とする指示を追記（改善案#1対応） |
| 2026-06-20 | Phase 0b（00b_goal_line_prfaq.md）を新設。PRFAQ下書き→コアジャーニー抽出→ゴールライン定義の流れとし、機能ごとの完成度に分散しないよう5観点でのレビュー・修正の繰り返しを組み込んだ。00_README.md／navigator_inception.md／navigator_prompt.md／SKILL.mdにもPhase 0bを反映（改善案#2対応） |
| 2026-06-20 | Phase 0c（00c_design_principles.md）を新設。対立軸の提案→優先する側と理由の回答→一覧化の流れとし、design_principles.mdを後続フェーズ全体（Phase1b・Phase3・Phase5等）の判断基準として参照する仕組みを組み込んだ。00_README.md／navigator_inception.md／navigator_prompt.md／SKILL.mdにもPhase 0cを反映（改善案#3対応） |
| 2026-06-20 | 改善案#4（XMLタグ構造の試験導入）は見送りと決定。XMLタグの効果は本来「未整形の議事録・メール等で指示と入力データの境界を明確にする」場面で出るが、本プロンプト集は既に〔作業ルール〕・STEP区切り・[Question]形式など独自の構造化記法で実運用を通じて磨かれており、解決すべき具体的な誤読・不具合が確認されていないため、書き換えのリスクに対してメリットが小さいと判断 |
| 2026-06-20 | 改善案#5対応。「あなたの役割: あなたは〇〇です」の一文を全フェーズファイル冒頭に統一して追加（01・03・06は既存。00・00b・00c・01b・01c・01d・02・04・05に新規追加）。レビュー時点で「対象（参照ファイル）」「条件（作業ルール）」は既に各ファイルで機能していたため、ばらつきが大きかった「役割」のみを補強する方針とした |
| 2026-06-20 | 改善案#6対応。00_README.mdに「困ったときのテクニック: プロンプトの自己最適化」セクションを新設。Claudeに「このプロンプトを最も答えやすい形に書き直してください」と頼むメタ最適化を、全フェーズ共通の任意テクニックとして案内（個別フェーズファイルは変更せず、肥大化を避けた） |
| 2026-06-21 | whisky_appプロジェクトでの実運用（Phase 0〜1d）から得た知見をプロンプト集に反映。(1) 00_README.mdに「整合性確認のスコープ原則」を新設し、直前フェーズだけでなくPhase 1全体・状況に応じた遡及確認を能動的に行うルールを明記。01b/01c/01d・navigator系に参照を追加。(2) 00b_goal_line_prfaq.md・01_generate_user_stories_plan.mdに「後回し」項目への対応目安タイミング提案を追加。(3) 01_generate_user_stories_plan.mdの作業ルールにAC語彙統一（フラグ→状態）・エピック間データ依存チェックを追加。(4) 01b_check_user_stories_consistency.mdにAC品質指摘が他エピック拡張を要する場合がある旨と、申し送りタイミングの根拠提示を追加。(5) 01c_actor_review.mdにCRUD完全性チェック・削除3パターンガイドを追加。(6) 01d_multi_perspective_review.mdにLEGALペルソナの業界規制発見における価値の明記、複数ペルソナの共通抜け漏れのエピック化ガイドを追加。(7) 00c_design_principles.mdに、対立軸が数値確定した場合はgoal_line.mdに記録する判別ルールを追加 |
