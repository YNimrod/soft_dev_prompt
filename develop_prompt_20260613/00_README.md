...
| `00b_goal_line_prfaq.md` | Phase 0b | ゴールライン定義（PRFAQ・コアジャーニー） |
| `00c_design_principles.md` | Phase 0c | デザインプリンシプル定義（判断の優先順位） |
...
Phase 0  ──→ inception/system_overview.md          ★Phase1への引き継ぎ要件
         ──→ knowledge_log.md（追記）               ★ナレッジ記録
              ↓  [同一セッションで継続]
Phase 0b ──→ inception/goal_line.md                 ★PRFAQ・コアジャーニー・ゴールライン
         ──→ knowledge_log.md（追記）               ★ナレッジ記録
              ↓  [同一セッションで継続]
Phase 0c ──→ inception/design_principles.md        ★判断の優先順位（後続フェーズ全体で参照）
         ──→ knowledge_log.md（追記）               ★ナレッジ記録
              ↓  [コンテキストクリア]
...
|---|---|
| Phase 0 | （会話の冒頭からやり直し） |
| Phase 0b | `system_overview.md`（Phase 0 完成版） |
| Phase 0c | `system_overview.md` + `goal_line.md`（Phase 0b 完成版） |
...
| 7 | **成果物を確定する際は、整合性確認のスコープ原則に従って既存成果物との整合性を能動的に確認する**（次節参照） | 指示を待たずに矛盾・古い記述を発見し、後工程での手戻りを防ぐ |

---

## 整合性確認のスコープ原則

各フェーズの整合性確認は、**原則として直前フェーズの成果物をスコープとする**。
ただしPhase 1のように複数のサブフェーズ（1 → 1b → 1c → 1d）に分かれている場合は、
サブフェーズ単位ではなく**「Phase 1全体」を1つの単位**として扱う
（例: Phase 1cの整合性確認は、直前のPhase 1bだけでなく、Phase 1・1b・1cまでの
成果物全体をスコープにする）。

この原則はあくまでデフォルトであり、状況に応じてスコープを広げる
（さらに前のフェーズまで遡って整合性を確認する）・狭める、という判断を
都度行ってよい。スコープを広げて確認した結果、古い成果物に矛盾や更新漏れが
見つかった場合は、ユーザーに指摘してから該当ファイルを更新すること。
これは指示を待たずに自発的に行う。
...
### ② Phase 0b: ゴールライン定義（新設）

Phase 0 完了後、Phase 1 着手前に実施。PRFAQ（完成後の発表文）を下書きし、そこから
「ユーザーが最初に価値を体験する一本道（コアジャーニー）」を抽出、各機能の最小実装を
定義します。機能ごとの完成度で分散しないよう、5観点でのレビューと修正を繰り返してから
`goal_line.md` として確定し、Phase 1以降のスコープ判断基準として引き継がれます。

### ③ Phase 0c: デザインプリンシプル定義（新設）

Phase 0b 完了後、Phase 1 着手前に実施。このプロダクトで起こりがちな対立軸（速さ vs 機能性
など）を3〜5個提案し、どちらを優先するかと理由を一覧化します。`design_principles.md` は
Phase 1b・Phase 3・Phase 5 など後続フェーズ全体で、判断に迷った際の参照先として使われます。
...
| Phase 0b | ゴールライン設定で機能ごとの分散になりやすかったケースと修正パターン |
| Phase 0c | このプロダクト種別で頻出した対立軸とその優先判断パターン |
...
## 困ったときのテクニック: プロンプトの自己最適化

各フェーズのプロンプトを送る前に、回答が分かりにくい・狙った出力が
得られないと感じた場合は、Claudeに以下を頼んでから本来のプロンプトを送ると改善することがあります。

```
このプロンプトを、あなた（Claude）が最も正確に理解し実行しやすい形に
書き直してください。書き直したプロンプトだけを提示し、まだ実行しないでください。
```

提示された書き直し案を確認し、問題なければそれを実行してもらってください。
特に、変数（`{{ }}`）の意味が伝わっていない・指示の意図がずれていると感じた時に有効です。

> ℹ️ これは全フェーズ共通で使える任意のテクニックです。プロンプト自体は変更せず、
> 必要に応じてその場で使ってください。
